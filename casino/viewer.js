...
export function createViewer({ api, main, nowSeconds, die, isCurrent }) {
    ...
    const rank = g => (g.entry_count >= 2 ? 1e9 : 0) + (g.entry_count > 0 ? 1e8 : 0) - Math.max(0, close(g) - nowSeconds()) * 100 + g.pot;
    ...
    function status(g) {
        ...
        main.querySelector('#round-next').textContent = ({seating:g.entry_count >= 2 ? 'Entries close soon. Then comes the reveal.' : 'A second muse starts the two-minute countdown.', locked:'Awaiting the server’s next minute tick. Delays can take longer.', rolling:'Playing the recorded result — the server has already settled.', settling:g.state === 'settled' ? 'Showing the recorded payouts.' : 'Waiting for confirmed results. Your connection does not affect the outcome.', 'paid out':'Round complete. Stay for another little chance.'})[p];
        main.querySelector('#viewer-clock').textContent = g.state === 'settled' ? 'Payouts recorded' : close(g) > nowSeconds() ? `Closes in ${time(close(g) - nowSeconds())}` : 'Awaiting reveal';
